...
export const PlanItemCard = ({ 
  ...
  return (
    <Card className={`transition-shadow hover:shadow-md ${isLocked ? "opacity-60 bg-muted/10" : ""}`}>
      <CardHeader>
      ...
      <CardContent>
        <ul className="mb-4 space-y-2">
          {item.sub_topics.map((subTopic, idx) => {
            // Logic: 
            // 1. If chapter is locked, ALL subtopics are locked.
            // 2. If chapter is unlocked, subtopic 0 is unlocked.
            // 3. Subtopic N is locked if Subtopic N-1 is NOT completed.
            const previousSubtopic = item.sub_topics[idx - 1];
            const isSubtopicLocked = isLocked || (idx > 0 && !(previousSubtopic as any).completed);
              ...
              <li key={idx} className="flex flex-col gap-2">
                <div
                  className={`group flex items-center justify-between gap-3 rounded-lg border border-border p-3 
                    ${isSubtopicLocked ? "cursor-not-allowed bg-muted/30" : "hover:bg-muted cursor-pointer"}`}
                ...
                <div
                  className={`overflow-hidden transition-all duration-200 ${
                    isOpen && !isSubtopicLocked ? "max-h-96 opacity-100 py-2" : "max-h-0 opacity-0"
                  }`}
                >
        ...
        >
          ...
          {isLocked ? "Complete previous chapter to unlock" : "Start Chapter"}
        </Button>
      </CardContent>
    </Card>
  );
